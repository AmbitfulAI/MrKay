...
interface NoteDoc { title: string; category: PopulatedCategory; date: Date; excerpt: string; featuredImages?: string[]; contentBlocks?: ContentBlock[]; }
  ...
  return (
    <CategoriesProvider initial={categories}>
      <div style={{ padding: "40px 48px" }}>
        ...
        <NoteForm initialData={{ title: note.title, category: String(note.category._id), date: new Date(note.date).toISOString().slice(0, 10), excerpt: note.excerpt, featuredImages: note.featuredImages ?? [], contentBlocks: note.contentBlocks ?? [] }} id={id} />
      </div>
    </CategoriesProvider>
  );
}
