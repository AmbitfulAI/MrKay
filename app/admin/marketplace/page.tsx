...
export default async function AdminMarketplace() {
  await connectDB();
  const items = await Product.find().sort({ order: 1 }).lean<ProductRow[]>();
    ...
    <div style={{ padding: "40px 48px" }}>
      ...
      {items.length === 0 ? (
        ...
        <div style={{ border: "1px solid var(--surface-2)" }}>
          ...
          {items.map((item) => (
            <div key={String(item._id)} style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px 80px 80px 100px", padding: "16px 20px", borderBottom: "1px solid var(--surface-2)", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
                <Link href={`/admin/marketplace/${String(item._id)}`} style={{ fontSize: "0.72rem", color: "var(--gold)", fontFamily: "var(--font-body)", textDecoration: "none" }}>Edit</Link>
                <DeleteButton id={String(item._id)} endpoint="/api/admin/marketplace" />
              </div>
            </div>
          ))}
        </div>
