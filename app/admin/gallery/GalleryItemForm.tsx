...
export function GalleryItemForm({ initialData, id }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!isEdit && !imageUrl) { setError("Please upload an image."); return; }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: "640px" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div><label style={labelStyle}>Category</label><input value={form.category} onChange={set("category")} placeholder="e.g. Speaking" style={input} /></div>
        <div><label style={labelStyle}>Span</label>
