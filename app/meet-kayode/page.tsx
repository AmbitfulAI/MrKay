...
export const metadata = {
  ...
  description: "Operations leader, executive systems thinker, consultant, and coach. The architect behind the work — built inside growing organisations, not in a lecture hall.",
};
    ...
    <>
          ...
          <div className="flex flex-col">
            {[
              ...
              >
                ...
                <div>
                  {item.body.split("\n\n").map((para, pi) => (
                    <p key={pi} className="text-muted font-light" style={{ fontSize: "0.9rem", lineHeight: 1.95, marginBottom: pi < item.body.split("\n\n").length - 1 ? "20px" : 0 }}>
                      {para}
                    </p>
                  ))}
                </div>
      ...
      <section className="bg-surface border-b border-surface-2 s-pad">
        <div className="container">
          ...
          <span className="gold-rule mb-7" />
          <p className="text-muted font-light mb-12" style={{ fontSize: "0.9rem", lineHeight: 1.9, maxWidth: "600px" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[2px] bg-surface-2">
            {credentials.map((c) => (
              <div key={c.category} className="bg-surface" style={{ padding: "36px 40px" }}>
                <span className="eyebrow block mb-5" style={{ color: "var(--gold)" }}>{c.category}</span>
                <ul className="flex flex-col gap-3 list-none">
                  {c.items.map((item) => (
                    <li key={item} className="text-muted font-light" style={{ fontSize: "0.85rem", lineHeight: 1.8 }}>
                      {item}
                    </li>
                  ))}
                </ul>
      ...
      <section className="bg-bg border-b border-surface-2 s-pad">
        <div className="container">
          ...
          <h2 className="display text-text mb-12" style={{ fontSize: "clamp(1.8rem, 3vw, 2.8rem)", lineHeight: 1.15 }}>
            ...
            {principles.map((p) => (
              <div key={p.label} className="flex gap-5 items-start">
                ...
                <div>
                  ...
                  <p className="text-muted font-light" style={{ fontSize: "0.88rem", lineHeight: 1.9 }}>{p.body}</p>
                </div>
              </div>
            ))}
          </div>
