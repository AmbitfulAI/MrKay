...
export default async function EditTestimonial({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = (raw ?? null) as { _id: string; quote: string; clientName: string; clientContext: string; order?: number } | null;
  if (!item) notFound();
  ...
  return (
    <div style={{ padding: "40px 48px" }}>
      ...
      <TestimonialForm id={id} initialData={{ quote: item.quote, clientName: item.clientName ?? "", clientContext: item.clientContext ?? "", order: item.order?.toString() ?? "" }} />
    </div>
  );
}
