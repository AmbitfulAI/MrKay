...
interface Story { _id: string; code: string; title: string; sector: string; order?: number; }
export const revalidate = 0;

export default async function AdminSuccessStories() {
  const items: Story[] = await sanityClient.fetch(successStoriesQuery);
          ...
          {items.map((item) => (
            <div key={item._id} style={{ display: "grid", gridTemplateColumns: "60px 1fr 160px 80px 100px", padding: "16px 20px", borderBottom: "1px solid var(--surface-2)", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
                ...
                <DeleteButton id={item._id} endpoint="/api/admin/success-stories" />
              </div>
            </div>
          ))}
        </div>
